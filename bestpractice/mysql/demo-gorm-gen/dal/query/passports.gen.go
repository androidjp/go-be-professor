...
func newPassport(db *gorm.DB, opts ...gen.DOOption) passport {
	_passport := passport{}

	_passport.passportDo.UseDB(db, opts...)
	_passport.passportDo.UseModel(&model.Passport{})

	tableName := _passport.passportDo.TableName()
	_passport.ALL = field.NewAsterisk(tableName)
...
func (p passport) clone(db *gorm.DB) passport {
	p.passportDo.ReplaceConnPool(db.Statement.ConnPool)
	return p
}

func (p passport) replaceDB(db *gorm.DB) passport {
...
func (p passportDo) Session(config *gorm.Session) *passportDo {
...
func (p passportDo) Delete(models ...*model.Passport) (result gen.ResultInfo, err error) {
